...
def _classify_context(stdout: str, **loop_fields):  # type: ignore[no-untyped-def]
    ...
    return SimpleNamespace(
        ...
        config=SimpleNamespace(forge=SimpleNamespace(default_branch="main"), harness=None),
...
def test_the_graph_carries_the_measurement_from_classify_to_publish(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """The graph keeps only the keys `LoopState` declares. Returned by classify
    and read by publish, an undeclared `changed_paths` was dropped in between,
    so every locally published row claimed no files and no later session was
    ever told where an open candidate wrote."""
    ...
    monkeypatch.setattr(
        G.audit,
        "run",
        lambda _state: {"finding": {"status": "proposed", "risk": "medium", "title": "t"}},
    )
    monkeypatch.setattr(
        G.classify,
        "run",
        lambda _state: {"risk": "medium", "changed_paths": ["src/retry.py", "tests/t.py"]},
    )
    monkeypatch.setattr(
        G.publish,
        "park",
        lambda state: (seen.update(paths=state.get("changed_paths")), {"outcome": "blocked"})[1],
    )

    G.build().compile().invoke({"loop": "code", "worktree": "/w", "branch": "b", "dry_run": False})

    assert seen["paths"] == ["src/retry.py", "tests/t.py"]
