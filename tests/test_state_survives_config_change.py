...
def _prepare(monkeypatch, tmp_path: Path, results: list[str]):  # type: ignore[no-untyped-def]
    ...
    from touchstone.hosted import gates, runtime
    ...
    # Prepare also records the forge gates; these tests are about the state lookup.
    monkeypatch.setattr(gates, "collect", lambda _config, _forge: gates.ForgeGates("", {}))
