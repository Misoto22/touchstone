...
def resolve_harness(
    ...
    *,
    ...
    if declaration.mode == "embedded":
        return _resolve_embedded(config, target_checkout=target_checkout)
...
def _resolve_embedded(config: Config, *, target_checkout: Path | None) -> HarnessContext:
    assert config.harness is not None
    root = (target_checkout or config.repo_path).resolve()
...
def _resolve_external(
    ...
    *,
    ...
    target_rules = (target_checkout or config.repo_path) / "AGENTS.md"
    try:
