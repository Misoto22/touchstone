...
def resolve_harness(
    ...
    *,
    ...
    if declaration.mode == "embedded":
        return _resolve_embedded(
            config, target_checkout=target_checkout, executor=target, revision=revision
        )
...
def _safe_entrypoint(root: str, reference: str, executor: Executor) -> Path:
    ...
    # `exists` is satisfied by a directory, and a directory names no rules. Ask for a regular
    # file, and treat an unanswerable probe the same way the symlink walk does.
    regular = executor.run(["test", "-f", candidate.as_posix()], timeout=30)
    if regular.timed_out or regular.code not in (0, 1):
        raise HarnessResolutionError(
            "harness-unreachable",
            f"could not determine whether {reference} is a regular file: {regular.tail()}",
        )
    if not regular.ok:
        raise HarnessResolutionError(
            "harness-entrypoint-missing",
            f"Harness entrypoint is not a regular file: {reference}",
        )
...
def _entrypoint_at_revision(root: str, reference: str, revision: str, executor: Executor) -> str:
    """The entrypoint as the named revision holds it, without building a worktree.

    A run creates its worktree from the fetched default branch, not from whatever the clone has
    checked out, so a pre-flight check that reads the working tree can pass while the run blocks.
    """

    listed = executor.run(
        ["git", "-C", root, "ls-tree", "--full-tree", "-z", revision, "--", reference],
        timeout=60,
    )
    if not listed.ok:
        raise HarnessResolutionError(
            "harness-unreachable",
            f"could not read {revision} in the execution repository: {listed.tail()}",
        )
    entry = listed.stdout.split("\x00", 1)[0].split("\t", 1)[0].split()
    # `<mode> <type> <object>`; 100644 and 100755 are the regular-file modes, 120000 a symlink.
    if len(entry) < 3 or entry[1] != "blob" or entry[0] not in ("100644", "100755"):
        raise HarnessResolutionError(
            "harness-entrypoint-missing",
            f"Harness entrypoint is not a regular file at {revision}: {reference}",
        )
    shown = executor.run(["git", "-C", root, "show", f"{revision}:{reference}"], timeout=60)
    if not shown.ok:
        raise HarnessResolutionError(
            "harness-entrypoint-missing",
            f"Harness entrypoint could not be read at {revision}: {reference}",
        )
    return shown.stdout
...
def _resolve_embedded(
    ...
    *,
    ...
    if revision is not None:
        entrypoint = PurePosixPath(root) / config.harness.entrypoint
        text: str | None = _entrypoint_at_revision(
            root, config.harness.entrypoint, revision, executor
        )
    else:
        entrypoint = PurePosixPath(_safe_entrypoint(root, config.harness.entrypoint, executor))
