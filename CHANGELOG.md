...
- An audit session is told which files each unmerged candidate already edits, so two runs that pick different defects stop landing in the same file. Suppressed titles already kept a session from raising a defect twice; they said nothing about where the earlier session wrote, and an unmerged branch is not in the new worktree for git to warn about. Whichever pull request merged second arrived conflicted, and both read as clean until then. Merged candidates are excluded: their edits are in the base branch, so they cannot collide.
...
- Generated stack evidence now scopes every configured Loop, not one named `code`. The generated `[loop]` table was written for that single name, so a project whose Loops were called anything else received no protected paths, no source confinement and no rendered brief context: `$project` fell back to "this repository" and the classify checks to the built-in protected list. Scoping such a Loop meant copying the generated values into `touchstone.toml` by hand, where `profile refresh` could never update them again. A project with a single `code` Loop regenerates byte-identically, and the table is emitted in sorted order so Loop declaration order cannot move the source digest.
...
- The built-in code-audit brief requires a session to name the sibling instances it is leaving unfixed, by file and symbol, and forbids a proof test that leaves shared state behind. One run named the two modules its diff did not reach and a reader knew where they stood; another fixed one of four copies of a status vocabulary under a title that read like all four, which is the title the next run is then told has been handled.
