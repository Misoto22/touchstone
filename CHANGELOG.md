...
- The hardcode Loop now requires a configuration extraction to be tested with a non-default value through the runtime consumer. A test that only repeats the unchanged default no longer counts as proof that the runtime reads configuration.
